...
test('creates and pushes an annotated tag from package.json version', () => {
  ...
  try {
    ...
    assert.match(
      run('git', ['for-each-ref', 'refs/tags/v0.2.0', '--format=%(contents)'], repositoryDirectory),
      /Release v0\.2\.0/,
    );
    assert.match(
      run('git', ['--git-dir', remoteDirectory, 'show-ref', '--tags'], tempDirectory),
      /refs\/tags\/v0\.2\.0/,
    );
  } finally {
...
test('creates an annotated tag when the repository has no Git identity', () => {
  ...
  try {
    ...
    run(
      'git',
      ['remote', 'add', 'origin', remoteDirectory],
      repositoryDirectory,
      isolatedGitEnvironment,
    );
    ...
    assert.equal(
      run(
        'git',
        ['for-each-ref', 'refs/tags/v0.2.1', '--format=%(taggername)'],
        repositoryDirectory,
        isolatedGitEnvironment,
      ).trim(),
      'github-actions[bot]',
